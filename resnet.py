...
def resnet34(num_classes=7, include_top=True, pretrained=False):
    # https://download.pytorch.org/models/resnet34-333f7ec4.pth
    model = ResNet(BasicBlock, [3, 4, 6, 3], num_classes=num_classes, include_top=include_top)
    if pretrained:
        state_dict = torch.load("pretrain_weights/resnet34-pre.pth")
        del_key = ["fc.weight", "fc.bias"]
        for k in del_key:
            del state_dict[k]
        print("load pretrained weight from {}".format("pretrain_weights/resnet34-pre.pth"))
        model.load_state_dict(state_dict, strict=False)
    return model


def resnet50(num_classes=7, include_top=True, pretrained=False):
    # https://download.pytorch.org/models/resnet50-19c8e357.pth
    model = ResNet(Bottleneck, [3, 4, 6, 3], num_classes=num_classes, include_top=include_top)
    if pretrained:
        state_dict = torch.load("pretrain_weights/resnet50-pre.pth")
        del_key = ["fc.weight", "fc.bias"]
        for k in del_key:
            del state_dict[k]
        print("load pretrained weight from {}".format("pretrain_weights/resnet50-pre.pth"))
        model.load_state_dict(state_dict, strict=False)
    return model


def resnet101(num_classes=7, include_top=True, pretrained=False):
